...
def init_clsfr(model:str="cca", tau_ms:int=None, offset_ms:int=0, fs:float=100, evtlabs:list=None, rank:int=1, center:bool=True, C:float=1.0, max_iter:int=500, **kwargs):
    ...
    if isinstance(model,BaseSequence2Sequence):
        ...
        clsfr = LinearSklearn(tau_ms=tau_ms, offset_ms=offset_ms, fs=fs, evtlabs=evtlabs, clsfr=Ridge(alpha=C,fit_intercept=True, max_iter=max_iter), **kwargs)
    elif model.lower() == 'lr':
        clsfr = LinearSklearn(tau_ms=tau_ms, offset_ms=offset_ms, fs=fs, evtlabs=evtlabs, clsfr=LogisticRegression(C=C,fit_intercept=True, max_iter=max_iter, solver='lbfgs'), labelizeY=True, **kwargs)
    elif model.lower() == 'svr':
        clsfr = LinearSklearn(tau_ms=tau_ms, offset=offset_ms, fs=fs, evtlabs=evtlabs, clsfr=LinearSVR(C=C, max_iter=max_iter), **kwargs)
    elif model.lower() == 'svc':
        clsfr = LinearSklearn(tau_ms=tau_ms, offset_ms=offset_ms, fs=fs, evtlabs=evtlabs, clsfr=LinearSVC(C=C, max_iter=max_iter), labelizeY=True, **kwargs)
    elif isinstance(model,BaseEstimator):
        clsfr = LinearSklearn(tau_ms=tau_ms, offset_ms=offset_ms, fs=fs, evtlabs=evtlabs, clsfr=model, labelizeY=True, **kwargs)
        ...
        clsfr = import_and_make_class(model,tau_ms=tau_ms, offset_ms=offset_ms, fs=fs, rank=rank, evtlabs=evtlabs, center=center, **kwargs)
    else:
...
class BaseSequence2Sequence(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, evtlabs=None, tau_ms:int=None, tau:int=None, offset_ms:int=None, offset:int=None, fs:float=None, outputscore:str='ip',
                priorweight:float=0, startup_correction:int=50, prediction_offsets:list=None, centerY:bool=False, 
                minDecisLen:int=0, bwdAccumulate:bool=False, nvirt_out:int=-20, nocontrol_condn:float=.5, verb:int=0):
        ...
        self.fs, self.tau_ms, self.tau, self.offset_ms, self.offset, self.priorweight, self.startup_correction, self.prediction_offsets = (fs, tau_ms, tau, offset_ms, offset, priorweight, startup_correction, prediction_offsets)
        self.verb, self.minDecisLen, self.bwdAccumulate, self.nvirt_out, self.nocontrol_condn, self.centerY = (verb, minDecisLen, bwdAccumulate, nvirt_out, nocontrol_condn, centerY)
    ...
    def stim2event(self, Y_TSy, prevY=None, fit=False):
        ...
        if self.evtlabs is not None:
            if fit or not hasattr(self,'s2estate_') or self.s2estate_ is None: # fit the event mapping
                Y_TSye, self.s2estate_, self.evtlabs_ = stim2event(Y_TSy, self.evtlabs, axis=-2, oM=prevY) # (tr, samp, Y, e)
    ...
    def predict(self, X, y, dedup0=False, prevY=None, offsets=None):
        ...
        Fy_MTSy = scoreOutput(Fe_MTSe, Y_TSye, dedup0=dedup0, R_mket=self.R_, offset=offsets, outputscore=self.outputscore) #(nM, nTrl, nSamp, nY)
    ...
    def decoding_curve(self, X_TSd, Y_TSy, cv=None, **kwargs):
        ...
        return decodingCurveSupervised(Fy_TSy, marginalizedecis=True, minDecisLen=self.minDecisLen, bwdAccumulate=self.bwdAccumulate, priorsigma=(self.sigma0_,self.priorweight), softmaxscale=self.softmaxscale_, nEpochCorrection=self.startup_correction, **kwargs)
    ...
    def transform_cv(self,X_TSd, Y_TSy, cv=None):
        if cv is not None:
            ...
            for fi,(train_idx, val_idx) in enumerate(cv):
                ...
                if Fei.ndim>3: 
                    if Fei.shape[0]>1: print("Warning: extra models ignored!")
                    Fei = Fei[0,...]
                if fi == 0:
    ...
    def confusion_matrix(self,X_TSd, Y_TSy, cv=None, prevY=None, ignore_unlabelled=True, normalize='true', score_noise=1e-5):
    ...
    def cv_fit(self, X, y, cv:int=5, fs=None, fit_params:dict=dict(), verbose:bool=0, return_estimator:bool=True, 
        ...
        self.softmaxscale_ = self.calibrate_softmaxscale(Fy_TSy) if calibrate_softmax and np.any(Y_TSy[:,:,0,...]) else 3
    ...
    def estimate_prior_sigma(self,FY_TSy):
        ...
        if self.verb>0: print('Sigma0 = {}'.format(sigma0))
        return sigma0
    ...
    def calibrate_softmaxscale(self,Fy_TSy):
        ...
        if self.nvirt_out is not None:
            ...
            if self.verb>0 : print("Added {} virtual outputs for calibration".format(nvirt_out))
            Fy_TSy = np.append(Fy_TSy,virt_Fy,axis=-1)
        nFy,_,_,_,_ = normalizeOutputScores(Fy_TSy, minDecisLen=-10, nEpochCorrection=self.startup_correction, priorsigma=(self.sigma0_,self.priorweight))
    ...
    def plot_model(self, fs:float=None, ncol:int=2, plot_pattern:bool=True, **kwargs):
        ...
        if hasattr(self,'R_') and not self.R_ is None:
            ...
            sp = (self.A_, 'spatial-pattern') if plot_pattern and hasattr(self,'A_') and self.A_ is not None else (self.W_, 'spatial-filter')
            tp = (self.I_, 'temporal-pattern') if plot_pattern and hasattr(self,'I_') and self.I_ is not None else (self.R_, 'temporal-filter')
            plot_factoredmodel(sp[0].reshape((-1,sp[0].shape[-1])),tp[0].reshape((-1,)+tp[0].shape[-2:]), evtlabs=evtlabs, offset=self.offset_, fs=fs, spatial_filter_type=sp[1], temporal_filter_type=tp[1], ncol=ncol, **kwargs)
        else:
...
#
#-----------------------------------------------------------
def get_rbf_basis(length,width=5,step=None,windowed:bool=False):
    step = 1 if step is None else step
    step = max(1,int(step)) # min step is 1
    cents = np.arange(0,length,step)
    # remove edge centers to allow trend to 0 at edges
    if windowed:
        cents=cents[1:-1]
    basis_bf = []
    for c in cents:
        basis = np.exp( -.5*(np.arange(0,length) - c)**2/(width)**2 )
        basis_bf.append(basis)
    basis_bf = np.stack(basis_bf,0)
    # unit-l2-norm each basis
    basis_bf = basis_bf / np.sqrt(np.sum(basis_bf*basis_bf,-1,keepdims=True))
    return basis_bf

def get_2drbf_basis(length,width=5,step=None,windowed:bool=False):
    if not hasattr(length,'__iter__'): length=[length] #ensure iterable
    if not hasattr(width,'__iter__'): width=[width] # ensure iterable
    if len(width)<len(length): # pad length 
        width = width + [width[-1]]*(len(length)-len(width))
    if step is None: step = [1 for w in width] # set step
    step = [max(1,int(s)) for s in step] # ensure int
    # get centers of each rbf
    if windowed:
        # remove edge centers to allow trend to 0 at edges
        cents_kld = np.meshgrid(np.arange(step[0],length[0]-step[0],step[0]), 
                                np.arange(step[1],length[1]-step[1],step[1]), indexing='ij')
        cents_kld = np.stack(cents_kld,-1)
    else:
        cents_kld = np.meshgrid(np.arange(0,length[0],step[0]),  np.arange(0,length[1],step[1]), indexing='ij')
        cents_kld = np.stack(cents_kld,-1)
    # get coords of each point in the space
    pts_ijd = np.stack(np.meshgrid(np.arange(length[0]), np.arange(length[1]), indexing='ij'),-1) #shape,2
    # compute the basis mapping, i.e. weight of each center at each point in the space
    basis_klij = np.exp( -.5*np.sum((pts_ijd - cents_kld[:,:,np.newaxis,np.newaxis,:])**2/np.maximum(np.array(width)**2,1e-8),-1) )
    # # plot the basis.
    # basis_klij = np.moveaxis(basis_klij,(0,1),(1,0)) # swap order to make plots easier to see
    # import matplotlib.pyplot as plt
    # fig,axes = plt.subplots(basis_klij.shape[0],basis_klij.shape[1])
    # for i in range(basis_klij.shape[0]):
    #     for j in range(basis_klij.shape[1]): 
    #         axes[i,j].imshow(basis_klij[i,j,:,:],aspect='auto')
    # plt.show()
    # flatten out the multi-dimensional aspect
    basis_bf = np.reshape(basis_klij,(basis_klij.shape[0]*basis_klij.shape[1],basis_klij.shape[2]*basis_klij.shape[3]))
    # unit-norm each basis
    basis_bf = basis_bf / np.sqrt(np.sum(basis_bf*basis_bf,-1,keepdims=True))
    return basis_bf


def get_fourier_basis(length,maxfreq=None,minfreq=1):
    if maxfreq is None: maxfreq = length//2
    if minfreq is None: minfreq = 1
    basis_bf=[]
    for n_cycles in range(minfreq,maxfreq):
        basis_bf.append( np.sin(np.linspace(0,2*np.pi,length)*n_cycles) )
        basis_bf.append( np.cos(np.linspace(0,2*np.pi,length)*n_cycles) )
    basis_bf = np.stack(basis_bf,0)
    # unit-norm each basis
    basis_bf = basis_bf / np.sqrt(np.sum(basis_bf*basis_bf,-1,keepdims=True))
    return basis_bf

def get_edge_suppression_window(length,edge_fraction):
    temporal_basis = np.ones(length,dtype=np.float32)
    edge_fraction = int(edge_fraction*length)
    if edge_fraction > 0:
...
def get_step_basis(length):
    basis_bf = np.zeros((length,length),dtype=np.float32)
...
def get_temporal_basis(tau:int, basis_type:str, dtype=np.float32) -> np.ndarray:
    ...
    Args:
        ...
        basis_type (str): string basis name, or given basis
        dtype ([type], optional): type of the basis map. Defaults to np.float32.

    Raises:
        ValueError: [description]

    Returns:
        np.ndarray: basis_bt matrix mapping from time, t, to basis space, b 
    """
    temporal_basis_bt = None

    # treat 2-d as stack of 1d basis
    if hasattr(tau,'__iter__') and len(tau)==2 and \
        not ( isinstance(basis_type,str) and basis_type.startswith('2d') ):
        raw_bt = get_temporal_basis(tau[-1],basis_type,dtype)
        if raw_bt is None: return raw_bt
    ...
    if isinstance(basis_type,np.ndarray):
        ...
        if basis_type.endswith('.json'): # load as matrix from file
            ...
            temporal_basis_bt = get_2drbf_basis(tau, width, step=step, windowed=('wrfb' in basis_type))

        elif basis_type.startswith('fourier'):
            try:
                freqs = basis_type[len('fourier'):]
                freqs = [int(e) for e in freqs.replace(',', '_').split('_')]
                minfreq, maxfreq = freqs[:2] if len(freqs)>1 else (1,freqs[0])
            except:
                minfreq, maxfreq = None, None
            temporal_basis_bt = get_fourier_basis(tau, maxfreq, minfreq)

        elif basis_type.startswith('winfourier') or basis_type.startswith('wf'):
            try:
                if basis_type.startswith('wf'):
                    freqs = basis_type[len('wf'):]
                else:
                    freqs = basis_type[len('winfourier'):]
                freqs = [int(e) for e in freqs.replace(',', '_').split('_')]
                minfreq, maxfreq = freqs[:2] if len(freqs)>1 else (None,freqs[0])
            except:
               minfreq, maxfreq = None, None
            temporal_basis_bt = get_fourier_basis(tau, maxfreq, minfreq)
            ...
            # unit-norm each basis
            temporal_basis_bt = temporal_basis_bt / np.sqrt(np.sum(temporal_basis_bt*temporal_basis_bt,-1,keepdims=True))
...
def apply_temporal_basis(temporal_basis, Cxx_dd, Cyx_yetd, Cyy_yetet):
    temporal_basis_bt = get_temporal_basis(Cyx_yetd.shape[-2],temporal_basis, Cxx_dd.dtype)
    if temporal_basis_bt is not None:
        if temporal_basis_bt.ndim==1:
            Cyx_yetd = Cyx_yetd * temporal_basis_bt[:,np.newaxis]
            Cyy_yetet = temporal_basis_bt[:,np.newaxis,np.newaxis] * Cyy_yetet * temporal_basis_bt
        elif temporal_basis_bt.ndim==2:
            Cyx_yetd = np.einsum("yetd,bt->yebd", Cyx_yetd, temporal_basis_bt)
            Cyy_yetet = np.einsum("bt,yetfu,cu->yebfc",temporal_basis_bt,Cyy_yetet,temporal_basis_bt)
        elif temporal_basis_bt.ndim==3:
            Cyx_yetd = np.einsum("yetd,bet->yebd", Cyx_yetd, temporal_basis_bt[:,:Cyx_yetd.shape[1],:])
            Cyy_yetet = np.einsum("bet,yetfu,ceu->yebfc",temporal_basis_bt[:,:Cyx_yetd.shape[1],:],Cyy_yetet,temporal_basis_bt[:,:Cyx_yetd.shape[1],:])
    return Cxx_dd, Cyx_yetd, Cyy_yetet, temporal_basis_bt

def apply_temporal_basis_X_TStd(temporal_basis, X_TStd):
    temporal_basis_bt = get_temporal_basis(X_TStd.shape[-2],temporal_basis, X_TStd.dtype)
    if temporal_basis_bt is not None:
        if temporal_basis_bt.ndim==1:
            X_TStd = X_TStd * temporal_basis_bt[:,np.newaxis]
        elif temporal_basis_bt.ndim==2:
            X_TStd = np.einsum("TStd,bt->TSbd", X_TStd, temporal_basis_bt)
        elif temporal_basis_bt.ndim==3:
...
def invert_temporal_basis_mapping(temporal_basis_bt, R_Mket):
    if temporal_basis_bt is not None : # map from feature space back to tau for direct application
        if temporal_basis_bt.ndim==1: # window function
            R_Mket = R_Mket * temporal_basis_bt
        elif temporal_basis_bt.ndim==2: # basis transformation
            R_Mket = np.einsum("...b,bt->...t",R_Mket,temporal_basis_bt)
        elif temporal_basis_bt.ndim==3:
            R_Mket = np.einsum("...eb,bet->...et",R_Mket,temporal_basis_bt)
    return R_Mket

def invert_temporal_basis_mapping_spatiotemporal(temporal_basis_bt, W_etd):
    if temporal_basis_bt is not None : # map from feature space back to tau for direct application
        if temporal_basis_bt.ndim==1: # window function
            W_etd = W_etd * temporal_basis_bt[:,np.newaxis]
        elif temporal_basis_bt.ndim==2: # basis transformation
            W_etd = np.einsum("...bd,bt->...td",W_etd,temporal_basis_bt)
        elif temporal_basis_bt.ndim==3:
...
class MultiCCA(BaseSequence2Sequence):
    ...
    def __init__(self, rank=1, reg=(1e-8,1e-8), rcond=(1e-4,1e-8), badEpThresh=6, badWinThresh=3, symetric=True, 
        ...
        super().__init__(evtlabs=evtlabs, tau=tau,  offset=offset, tau_ms=tau_ms, offset_ms=offset_ms, fs=fs, outputscore=outputscore, **kwargs)
    ...
    def fit_cca(self, Cxx_dd, Cyx_yetd, Cyy_yetet, rank=None, reg=None, rcond=None, CCA=None, symetric=None, whiten_alg=None, temporal_basis=None):
        if rank is None: rank = self.rank
        if reg is None:  reg  = self.reg
        if rcond is None: rcond = self.rcond
        if CCA is None: CCA=self.CCA
        if symetric is None: symetric = self.symetric
        if whiten_alg is None: whiten_alg = self.whiten_alg
        if temporal_basis is None: temporal_basis = self.temporal_basis

        # transform the temporal basis used
        Cxx_dd, Cyx_yetd, Cyy_yetet, temporal_basis_bt = apply_temporal_basis(temporal_basis, Cxx_dd, Cyx_yetd, Cyy_yetet)

        #print(" rank = {} reg={}".format(rank,reg))
        #print(" Cxx={} Cyx={} Cyy={}".format(Cxx_dd.shape, Cyx_yetd.shape, Cyy_yetet.shape))
        J, W_Mkd, R_Mket, A_Mkd, I_Mket = multipleCCA(Cxx_dd, Cyx_yetd, Cyy_yetet, reg=reg, rank=rank, rcond=rcond, CCA=CCA, symetric=symetric, whiten_alg=whiten_alg)
        # strip the model dim
    ...
    def fit(self, X, y, stimTimes=None, fs=None, calibrate_softmaxscale:bool=False):
        ...
        Y_true = Y_TSye[:, :, 0:1, ...] if np.any(Y_TSye[:,:,0,:]) else np.sum(Y_TSye,2,keepdims=True) #  (tr,samp,1,e)
        # get summary statistics
        #print("X={} |X|={}".format(X.shape,np.sum(X**2,axis=(0,1))))
        Cxx, Cyx, Cyy = updateSummaryStatistics(X_TSd, Y_true, stimTimes, tau=self.tau_, offset=self.offset_, badEpThresh=self.badEpThresh, badWinThresh=self.badWinThresh, center=self.center, cxxp=self.cxxp)
...
class MultiCCACV(MultiCCA):
    def __init__(self, inner_cv:int=5, inner_cv_params:dict=None,
        ...
        """        
        super().__init__(rank=rank, reg=reg, rcond=rcond, badEpThresh=badEpThresh, symetric=symetric, 
                        center=center, CCA=CCA, outputscore=outputscore, whiten_alg=whiten_alg, cxxp=cxxp, temporal_basis=temporal_basis, 
                        evtlabs=evtlabs, tau=tau, offset=offset, tau_ms=tau_ms, offset_ms=offset_ms, fs=fs, normalize_sign=normalize_sign, **kwargs)
    ...
    def fit(self,X,Y,fs=None,inner_cv_params:dict=None,retrain_on_all:bool=True):
        ...
        Returns:
            [type]: [description]
        """        
        if inner_cv_params is None: inner_cv_params = self.inner_cv_params
    ...
    def cv_fit(self, X, Y, cv=None, fs=None, verbose:bool=0,  score_type:str='audc',
        ...
        if ranks is None: ranks=(self.rank,)
        ...
        # compute the summary statistics for each trial
        Cxxs, Cyxs, Cyys = self.per_trial_summaryStatistics(X_TSd, Y_true, self.tau_, self.offset_, center=self.center, cxxp= self.cxxp, badEpThresh=self.badEpThresh, badWinThresh=self.badWinThresh)

        # Rank is special hyper-param
        if ranks is None:
            if fit_params is not None:
                ranks = fit_params.get('rank',None)
                # dict comphrension to make copy w/o rank as key
                fit_params = { k:v for k,v in fit_params.items() if not k=='rank'}
            if ranks is None:
                ranks = [self.rank]
        ...
        if self.verb>0 : print("CV over {} ranks {} and\n {} configs: {}".format(len(ranks),ranks,len(fit_configs),[c for c in fit_configs]))
        ...
        if self.verb>0:
            for ci,c in enumerate(fit_configs):
                print("Cfg {}: Rank: ".format(c) + ", ".join(["{}={:4.3f}".format(r,avescore[ci,ri]) for ri,r in enumerate(ranks)]))
        ...
        if self.sigma0_ is None: self.sigma0_ = 1
        if self.softmaxscale_ is None: self.softmaxscale_=3
        ...
        return dict(estimator=Fy, estimator_cv=Fy_cv, test_score=scores_cv[maxci][maxri], scores_cv=scores_cv, ranks=ranks, fit_configs=[cf for cf in fit_configs])
...
class MultiCCA2(MultiCCA):
    ...
    def fit(self, X, y, stimTimes=None, fs=None, calibrate_softmaxscale:bool=False):
        ...
        # get summary statistics
        #print("X={} |X|={}".format(X.shape,np.sum(X**2,axis=(0,1))))
        Cxx_dd, Cyx_yetd, Cyy_yetet = updateSummaryStatistics(X_TSd, Y_true, stimTimes, tau=self.tau_, offset=self.offset_, badEpThresh=self.badEpThresh, center=self.center)
...
class FwdLinearRegression(BaseSequence2Sequence):
    ...
    def __init__(self, evtlabs=None, tau=None, offset=None, tau_ms=None, offset_ms=0, fs=None, reg=None, rcond=1e-6, badEpThresh=6, center=True, **kwargs):
...
class FwdLinearRegressionCV(FwdLinearRegression):
    ...
    def __init__(self, inner_cv:int=5, inner_cv_params:dict=None, 
                 ...
                 **kwargs):
        super().__init__(evtlabs=evtlabs,tau=tau,offset=offset,tau_ms=tau_ms,offset_ms=offset_ms,fs=fs,reg=reg,badEpThresh=badEpThresh,center=center,**kwargs)
    ...
    def cv_fit(self, X, Y, cv=None, fs=None, verbose:bool=0,
        ...
        if fit_params is None or len(fit_params)==0:
            # call the base version
            return super().cv_fit(self, X, Y, cv=cv, fs=fs, verbose=verbose, 
                            return_estimator=return_estimator, calibrate_softmax=calibrate_softmax,  retrain_on_all=retrain_on_all)
        ...
        for ti in range(X.shape[0]):
            ...
            Cyx_yetd = updateCxy(None, X[ti:ti+1,...], Y_TSe[ti:ti+1,:,np.newaxis,:], None, tau=self.tau_, offset=self.offset_)
            Cyx_ted  = np.moveaxis(Cyx_yetd[0,...],(0,1,2),(1,0,2)) # yetd -> ted
        ...
        if self.verb>0 : print("CV over {} configs: {}".format(len(fit_configs),fit_configs))
        ...
        return dict(estimator=Fy, score=scores_cv[maxci], estimator_cv=Fy_cv, scores_cv=scores_cv, fit_configs=[cf for cf in fit_configs])
...
class BwdLinearRegression(BaseSequence2Sequence):
    ...
    def __init__(self, evtlabs=None, tau=None, offset=None, tau_ms=None, offset_ms=0, fs=None, reg=None, badEpThresh=6, center=True, compdiagcxx:bool=True, temporal_basis:str=None, **kwargs):
        super().__init__(evtlabs=evtlabs, tau=tau, offset=offset, tau_ms=tau_ms, offset_ms=offset_ms, fs=fs, **kwargs)
    ...
    def fit_bwd(self, Cxx_tdtd, Cxy_tde, reg:float=None, rcond:float=None, verb:int=0):
        ...
        if verb>0: print("Cxx={}  Cxy={}".format(Cxx_td_td.shape,Cxy_td_e.shape))
    ...
    def fit(self, X, y, fs=None):
        ...
        if self.compdiagcxx:
            ...
            Cxy_tyed, taus  = compCyx_diag(X, Y_TSe[:,:,np.newaxis,:], tau=[1,self.tau_], offset=self.offset_) # (tau,d,1,e) shifted X with Y
...
class BwdLinearRegressionCV(BwdLinearRegression):
    ...
    def __init__(self, inner_cv:int=5, inner_cv_params:dict=None, 
                 ...
                 **kwargs):
        super().__init__(evtlabs=evtlabs,tau=tau,offset=offset,tau_ms=tau_ms,offset_ms=offset_ms,fs=fs,reg=reg,badEpThresh=badEpThresh,center=center,compdiagcxx=compdiagcxx,**kwargs)
        self.inner_cv, self.inner_cv_params = (inner_cv, inner_cv_params)
    ...
    def cv_fit(self, X, Y, cv=None, fs=None, verbose:bool=0,
        ...
        for ti in range(X.shape[0]):
            ...
            if self.compdiagcxx:
                ...
                Cxx_tdd  = compCxx_diag(X_TSd[ti:ti+1,...], tau=self.tau_) # (tau,d,tau,d) cross-cov shifted X with itself
                Cxxs_tdtd.append(Cxx_tdd)

                Cxy_tyed, taus  = compCyx_diag(X_TSd[ti:ti+1,...], Y_true[ti:ti+1,...], tau=[1,self.tau_], offset=self.offset_) # (tau,d,1,e) shifted X with Y
                Cxy_tdey = np.moveaxis(Cxy_tyed,(0,1,2,3),(0,3,2,1))
                Cxy_tde  = Cxy_tdey.reshape(Cxy_tdey.shape[:3])
                Cxys_tde.append(Cxy_tde)
            else:
                Cxx_tdtd  = crossautocov(X_TSd[ti:ti+1,...], X_TSd[ti:ti+1,...], tau=(self.tau_, self.tau_)) # (tau,d,tau,d) cross-cov shifted X with itself
                Cxxs_tdtd.append(Cxx_tdtd)

                Cxy_1dte  = crossautocov(X_TSd[ti:ti+1,...], Y_true[ti:ti+1,:,0,:], tau=[1,self.tau_], offset=self.offset_) # (tau,d,1,e) shifted X with Y
                Cxy_tde1  = np.moveaxis(Cxy_1dte,(0,1,2,3),(3,1,0,2))
                Cxy_tde   = Cxy_tde1.reshape(Cxy_tde1.shape[:3])
                Cxys_tde.append(Cxy_tde)
...
class LinearSklearn(BaseSequence2Sequence):
    ...
    def __init__(self, clsfr="lr", clsfr_args:dict={}, evtlabs=None, tau=None, offset=None, tau_ms=None, offset_ms=0, fs=None, labelizeY=False, ignore_unlabelled=True, chunk_size:int=1, chunk_labeller:str=None, badEpThresh=None, temporal_basis:str=None, **kwargs):
        super().__init__(evtlabs=evtlabs, tau=tau, offset=offset, tau_ms=tau_ms, offset_ms=offset_ms, fs=fs, **kwargs)
    ...
    @staticmethod
    def make_sklearn_dataset(X_TSd, Y_TSe, tau, offset, chunk_size=1, labelizeY=False, ignore_unlabelled=True, chunk_labeller=None, badEpThresh=None, temporal_basis:str=None, verb=0):
        '''fit 2 multi-dim time series: X = (tr, samp, d), Y = (tr, samp, e) using a given sklearn linear_model method'''
        if chunk_size is None: chunk_size=1 # default to sliding window 1 sample at a time
        # slice X to get block of data per sample (with optional step-size)
        X_TStd = window_axis(X_TSd, winsz=tau, step=chunk_size, axis=1) if tau is not None and tau<X_TSd.shape[1] else X_TSd[:,np.newaxis,...]
        ...
        if labelizeY: # convert Y to a label list + with chunk_size
            ...
            if ignore_unlabelled:
                if np.any(Y_TS==0):
                    keep = Y_TS!=0
                    if verb > 0: print("Warning: removing {} unlabelled examples from {} training".format(np.sum(unlabelled),len(unlabelled)))
                    Y_TS = Y_TS[keep]
                    Y_TS_e = Y_TS_e[keep, ...]
    ...
    @staticmethod
    def sklearn_fit(X_TSd, Y_TSe, clsfr, tau, offset, chunk_size=1, labelizeY=False, ignore_unlabelled=True, chunk_labeller=None, badEpThresh=None, temporal_basis:str=None, verb=0, **fit_params):
        Xtrn, Ytrn, temporal_basis = LinearSklearn.make_sklearn_dataset(X_TSd,Y_TSe,tau,offset,chunk_size,labelizeY,ignore_unlabelled,chunk_labeller,badEpThresh,temporal_basis,verb)
        ...
        if verb > 0: print("Training score = {}".format(clsfr.score(Xtrn,Ytrn)))
        return clsfr
    ...
    def make_clsfr(self, clsfr, clsfr_args, C=1):
        ...
        if isinstance(clsfr,str):
            if clsfr.lower() == 'ridge': # should be equivalent to BwdLinearRegression
                if not 'fit_intercept' in clsfr_args: clsfr_args['fit_intercept']=True
                clsfr = Ridge(alpha=1/C, **clsfr_args)
            elif clsfr.lower() == 'lr':
                if not 'solver' in clsfr_args: clsfr_args['solver']='lbfgs'
                if not 'max_iter' in clsfr_args: clsfr_args['max_iter']=250
                if not 'fit_intercept' in clsfr_args: clsfr_args['fit_intercept']=True
    ...
    def fit(self, X, y, fs=None, C:float=1, C_scale:float=None, **fit_params):
        ...
        Xtrn, Ytrn, temporal_basis = LinearSklearn.make_sklearn_dataset(X,Y_true,tau=self.tau_, offset=self.offset_, labelizeY=self.labelizeY, ignore_unlabelled=self.ignore_unlabelled, badEpThresh=self.badEpThresh, chunk_size=self.chunk_size, chunk_labeller=self.chunk_labeller, temporal_basis=self.temporal_basis, **fit_params)
        ...
        # log the binary training set performance.
        if self.verb > 0: print("Training score = {}".format(self.score(Xtrn,Ytrn)))
    ...
    def cv_fit(self, X, Y, fs=None, cv=None, fit_params:dict=dict(), verbose:bool=0, 
               return_estimator:bool=True, calibrate_softmax:bool=True, retrain_on_all:bool=True, C_scale:float=None):
        ...
        if fit_params is None :
            # call the base version
            return BaseSequence2Sequence.cv_fit(self, X, Y, cv=cv_in, fs=fs, fit_params=fit_params, verbose=verbose, 
                            return_estimator=return_estimator, calibrate_softmax=calibrate_softmax,  retrain_on_all=retrain_on_all)
...
class LinearLogisticRegression(LinearSklearn):
    def __init__(self, evtlabs=None, tau=None, offset=None, tau_ms=None, offset_ms=0, fs=None, labelizeY=True, ignore_unlabelled=True, badEpThresh=None, **kwargs):
        super().__init__(clsfr='lr', evtlabs=evtlabs, tau=tau, offset=offset, tau_ms=tau_ms, offset_ms=offset_ms, fs=fs, labelizeY=labelizeY, ignore_unlabelled=ignore_unlabelled, badEpThresh=badEpThresh, **kwargs)

class LinearLogisticRegressionCV(LinearSklearn):
    def __init__(self, evtlabs=None, tau=None, offset=None, tau_ms=None, offset_ms=0, fs=None, labelizeY=True, ignore_unlabelled=True, badEpThresh=None, inner_cv=5, inner_cv_params:dict={"C":[8**v for v in [-6,-4,-2,-1,0,1,2,4,6,10,16]]}, **kwargs):
        super().__init__(clsfr='lr', evtlabs=evtlabs, tau=tau, offset=offset, tau_ms=tau_ms, offset_ms=offset_ms, fs=fs, labelizeY=labelizeY, ignore_unlabelled=ignore_unlabelled, badEpThresh=badEpThresh, **kwargs)
        self.inner_cv, self.inner_cv_params = (inner_cv, inner_cv_params)
...
class LinearRidgeRegression(LinearSklearn):
    def __init__(self, evtlabs=None, tau=None, offset=None, tau_ms=None, offset_ms=0, fs=None, labelizeY=False, ignore_unlabelled=False, badEpThresh=None, **kwargs):
        super().__init__(clsfr='ridge', evtlabs=evtlabs, tau=tau, offset=offset, tau_ms=tau_ms, offset_ms=offset_ms, fs=fs, labelizeY=False, ignore_unlabelled=False, badEpThresh=None,  **kwargs)
...
def testcase(dataset='toy',loader_args=dict()):
    ...
    bwd = BwdLinearRegression(tau=tau, evtlabs=evtlabs, verb=2, reg=None, badEpThresh=4, compdiagcxx=True, temporal_basis='winfourier5' )
    fit_predict_plot(bwd, X_TSd, Y_TSy, fs, title='bwd')


    lr = LinearLogisticRegression(tau=tau, offset=offset, evtlabs=evtlabs, labelizeY=True, ignore_unlabelled=False, temporal_basis='winfourier4')
    ...
    # cca - cv-fit
    print("CV fitted")
    cca = MultiCCACV(tau=tau, offset_ms=50, rank=1, reg=None, evtlabs=evtlabs, verb=1, nvirt_out=None, inner_cv_params=dict(rank=(1,2,3,5), reg=((1e-4,1e-4),(1e-3,1e-3),(1e-2,1e-2),(1e-1,1e-1))))
    ...
    t0=time.time()
    bwd = BwdLinearRegressionCV(tau=tau, evtlabs=evtlabs, verb=2, reg=None, badEpThresh=4, compdiagcxx=True, inner_cv_params=dict(reg=(0,1e-6,1e-4,1e-3,1e-2,1e-1,.5,1-1e-1,1-1e-2)))
    ...
    print("Backward Model")
    t0=time.time()
    bwd = BwdLinearRegressionCV(tau=tau, evtlabs=evtlabs, verb=2, reg=None, badEpThresh=4, inner_cv_params=dict(reg=(1e-4,1e-3,1e-2,1e-1,.5,1-1e-1,1-1e-2)))
    fit_predict_plot(bwd, X_TSd, Y_TSy, fs)
    print("in {}s".format(time.time()-t0))
    ...
    print("sklear-lr")
    lr = LinearSklearn(tau=tau, offset=offset, evtlabs=clsfrevtlabs, clsfr=LogisticRegression(C=1, multi_class='multinomial', solver='sag'), labelizeY=True)
    fit_predict_plot(lr, X_TSd, Y_TSy, fs)

    print("sklear-svc")
    svc = LinearSklearn(tau=tau, offset=offset, evtlabs=clsfrevtlabs, clsfr=LinearSVC(C=1, multi_class='ovr'), labelizeY=True)
