...
def erpViewer(ui: UtopiaDataInterface, timeout_ms: float=np.inf, tau_ms: float=500,
    ...
    while ui.getTimeStamp() < t0+timeout_ms:
        ...
        stimulus = rawstimulus[:,0:1] # (nsamp,1)
        ...
        for ei,lab in enumerate(evtlabs):
            ...
            if False:
                ...
                A=np.zeros((erp.shape[-1])); A[-1]=1 #(1,d)
                R=erp[:,-1:]# @ A
...
if __name__=='__main__':
    ...
    parser.add_argument('--evtlabs',type=str, help='comma separated list of stimulus even types to use', default=None)
