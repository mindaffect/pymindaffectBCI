...
class ScalarFunctionApplier(BaseEstimator):
    ...
    def fit(self,X,y=None):
        if isinstance(self.op,str):
            ...
            if fn is None:   fn = globals().get(self.op, None)
            if fn is None:   fn = getattr(locals().get("np"),self.op,None) if 'np' in locals() else None
            if fn is None:   fn = getattr(globals().get("np"),self.op,None) if 'np' in globals() else None
            self.op = fn
        return self
    def transform(self,X,y=None):
...
class TrialPlotter(BaseEstimator):
    ...
    def __init__(self, fs:float=None, plot_trial_number:int=0, plot_x:bool=True, plot_y:bool=True, plot_args:dict=dict(), plot_output_idx:int=None, fig:int=None, block:bool=False, suptitle:str=None):
        """[summary]
...
#--------------------------------------------------------------------------
import matplotlib.pyplot as plt
def plot_class_average(X_TSd, Y_TSye=None, y0_is_true:bool=True, fs=None, outputs=None, ch_names=None, evtlabs=None, 
                    tau=None, tau_ms=500, offset=None, offset_ms=None, fig=None, suptitle=None, **kwargs):

    # get sample rate, priority order is: fit-argument, X.info, __init__-argument
    if hasattr(X_TSd,'info'): fs = X_TSd.info['fs']
    if tau is None:
    ...
    plot_erp(Cxy_yetd, fs=fs, ch_names=ch_names, times=times, evtlabs=evtlabs, outputs=outputs, suptitle=suptitle, **kwargs)
    return fig
...
class EventRelatedPotentialPlotter(BaseEstimator):
    """plot the incomming trials """
    def __init__(self, fs:float=None, tau:int=None, tau_ms:float=500, offset:int=None, offset_ms:float=0, plot_trial_number:int=0, plot_args:dict=dict(), fig:int=None, block:bool=False, suptitle:str=None, cls_diff:bool=False):
        """[summary]
...
class SummaryStatisticsPlotter(BaseEstimator):
    """plot the incomming trials """
    def __init__(self, fs:float=None, tau:int=None, tau_ms:float=500, offset:int=None, offset_ms:float=0, plot_args:dict=dict(), fig:int=None, block:bool=False, suptitle:str=None):
        """[summary]
    ...
    def fit(self,X,y=None,y0_is_true:bool=True, fs=None, outputs=None, ch_names=None, evtlabs=None):
        # get sample rate, priority order is: fit-argument, X.info, __init__-argument
        if fs is not None: self.fs_=fs
        elif hasattr(X,'info'): self.fs_ = X.info['fs']
        else: self.fs_ = self.fs
        self.tau_ = self.tau 
...
class TimeShifter(BaseEstimator, ModifierMixin):
    ...
    def __init__(self, axis=1, shift_who:str='y', timeshift:int=None, timeshift_ms:float=None, fs:float=None, padding:str=0, verb=0):
    ...
    def fit(self,X, y=None, fs:float=None):
        ...
        if self.verb>0: print("TimeShifter: {} samples".format(self.timeshift_))
...
class TargetEncoder(BaseEstimator, ModifierMixin):
    ...
    def fit_modify(self,X, y=None, prevY=None):
        ...
        if self.verb>0 : print("Y_in={}".format(y.shape))
        if self.evtlabs is not None:
            y, self.s2estate_, self.evtlabs_ = stim2event(y, evtypes=self.evtlabs, axis=self.axis, oM=prevY, **self.kwargs) # (tr, samp, Y, e)
        else:
            self.e2state_, self.evtlabs_ = (self.evtlabs, None)
        if self.verb>0 : print("Y_out={}".format(y.shape))
...
class BadChannelRemover(BaseEstimator):
    ...
    def __init__(self, thresh:float=3, mode:str='remove', minthresh:float=4, disconnected_power:float=1e-3, max_power:float=1e8, center:bool=True, verb:int=0):
    ...
    def fit(self,X, y=None):
        ...
        if self.verb>0:
            ...
            if hasattr(X,'info') and 'ch_names' in X.info and X.info['ch_names'] is not None:
                print("{} bad-ch: {}".format(np.sum(isbad),[c for c,b in zip(X.info['ch_names'],isbad.ravel()) if b]))
            else:
...
class BadChannelInterpolator(BaseEstimator):
    ...
    def fit(self,X, y=None, ch_names:list=None, bad_ch:list=None, ch_pos:list=None):
        ...
        Args:
            X ([type]): (nTrial, nSample, d) time series data
        """
        if ch_names is None: ch_names = self.ch_names
        if bad_ch is None: bad_ch = self.bad_ch
        if ch_names is None:
...
def estimate_noise_subspace_decorrelation_filter(X_TSd,ch_idx,tol=5e-4,sample_idx=None):
    ...
    if sample_idx is None:
        ...
        if not isinstance(sample_idx,tuple) : sample_idx=(sample_idx,)
...
class NoiseSubspaceDecorrelator(BaseEstimator):
    ...
    def __init__(self, tol:float=5e-4, ch_idx:tuple=None, noise_idx:tuple='idOutliers', ch_names:tuple=None, filterband:list=None, verb:int=0):
        """ Decorrelate the data w.r.t a noise sub-space -- either a channel subset or a time subset
    ...
    def fit(self,X, y=None, ch_names:tuple=None, fs=None):
        ...
        if self.ch_idx_ is not None and len(self.ch_idx_)>1:
            if self.filterband is not None:
                ...
                X[...,self.ch_idx_], _, _ = butter_sosfiltfilt(X[...,self.ch_idx_],self.filterband,fs=fs,order=4,axis=-2)
            ...
            if isinstance(noise_idx,str) and noise_idx.startswith('idOutliers'):
                ...
                if self.verb>0 : print("{}/{} artifacts".format(np.sum(nidx_Tw),nidx_Tw.size))
...
class Slicer(BaseEstimator, ModifierMixin):
    ...
    def fit(self,X,y=None):
        ...
        for i,s in enumerate(tmp):
            #if s=='...':   tmp[i]=Ellipsis
            if isinstance(s,str):
                if s==':':     tmp[i]=slice(None)
                elif ":" in s:   tmp[i]=slice(*[int(i) for i in s.split(":")])
...
class Chunker(BaseEstimator, ModifierMixin):
    ...
    """
    def __init__(self, tau:int=None, tau_ms:float=None, offset:int=None, offset_ms:float=0, fs:float=None, chunk_size:int=None, chunk_size_ms:float=None, per_chunk_label:bool=False):
    ...
    def fit(self,X,y=None, fs=None):
        ...
        self.offset_ = self.offset if self.offset is not None else int(self.offset_ms * fs / 1000) if self.offset_ms is not None else 0
        self.chunk_size_ = self.chunk_size if self.chunk_size is not None else int(self.chunk_size_ms * fs / 1000) if self.chunk_size_ms is not None else 1
        return self
    ...
    def modify(self,X, y=None):
        ...
        X,y = chunk_data(X,y,tau=self.tau_,offset=self.offset_,chunk_size=self.chunk_size_,per_chunk_label=self.per_chunk_label)

        if info: X = InfoArray(X,info) # replace meta-info
        return X,y
...
class FFTfilter(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, axis=1, filterbank:list=None, fs:float=None, blksz=None, blksz_ms=None, window=None, fftlen:int=None, overlap:float=.5, squeeze_feature_dim:bool=False, prefix_feature_dim:bool=False, ola_filter:bool=True, verb=1):
    ...
    def fit(self,X, y=None, filterbank:list=None, fs:float=None, blksz:int=None, fftlen:int=None):
        ...
        if self.ola_filter:
            self.blksz_ = blksz if blksz is not None else int(self.blksz_ms * self.fs_ / 1000) if self.blksz_ms is not None else self.fs_/2 if self.fs_ is not None else 100 # default to 1/2 sec or 100 samples
        ...
        self.mask_bf_, self.bandtype_, self.freqs_ = pp.fftfilter_masks(self.fftlen_, filterbank, fs=self.fs_, dtype=X.dtype)
    ...
    def transform(self,X, y=None):
        ...
        if self.ola_filter:
            X_TSfd = pp.inner_ola_fftfilterbank(X_TSd, self.mask_bf_, self.bandtype_, self.window_, self.step_, axis=self.axis, prefix_band_dim=self.prefix_feature_dim)
        else:
            X_TSfd = pp.inner_fftfilterbank(X_TSd, self.mask_bf_, self.bandtype_, window=self.window_, axis=self.axis, prefix_band_dim=self.prefix_feature_dim)
...
def block_cov_mx(X_TSd,Y_TSy,axis,window,step,resample_y,mean:bool=False):
    ...
    Y_Tby =np.zeros(Y_TSy.shape[:axis]+(len(blkidxs),)+Y_TSy.shape[axis+1:]) if Y_TSy is not None else None# downsampled Y
    X_Tbdd=np.zeros(X_TSd.shape[:axis]+(len(blkidxs),)+X_TSd.shape[axis+1:]+(X_TSd.shape[-1],),dtype=X_TSd.dtype) # block cov downsampled X
...
class BlockCovarianceMatrixizer(BaseEstimator, ModifierMixin):
    ...
    def __init__(self, axis=1, fs:float=None, blksz:int=None, blksz_ms:float=None, resample_y='max', window:str=1, overlap:float=0, squeeze_feature_dim:bool=False, mean:bool=True, verb=1):
    ...
    def fit(self,X, y=None, fs:float=None, blksz:int=None, blksz_ms:float=None):
        ...
        self.blksz_ = int(self.blksz) if self.blksz is not None else int(self.blksz_ms * self.fs_ / 1000) if self.blksz_ms is not None else X.shape[self.axis]
...
class WelchPSD(BaseEstimator, ModifierMixin):
    def __init__(self,axis=-2,detrend:bool='constant', blksz=None, blksz_ms=None, fs:float=None, resample_y:str=None):
        self.axis, self.detrend, self.blksz, self.blksz_ms, self.fs, self.resample_y = (axis,detrend,blksz,blksz_ms,fs,resample_y)

    def fit(self,X,y=None,fs=None):
        self.fs_ = fs if fs is not None else X.info['fs'] if hasattr(X,'info') else self.fs
        self.blksz_ = int(self.blksz) if self.blksz is not None else int(self.blksz_ms * self.fs_ / 1000) if self.blksz_ms is not None else self.fs_//4
    ...
    def transform(self,X,y=None):
        info = X.info if hasattr(X,'info') else None
        freqs, X = welch(X, axis=self.axis, fs=self.fs_, nperseg=self.blksz_, return_onesided=True, detrend=self.detrend)
...
class CommonSpatialPatterner(BaseEstimator):
    ...
    def __init__(self, nfilt_per_class:int=3, mean:bool=True, compress_feature_dim=False, spoc:bool=False, reg:float=1e-6, rcond:float=1e-6, verb=1):
    ...
    def plot_model(self, evtlabs=None, plot_pattern:bool=True, ch_names=None, **kwargs):
        sp = (self.A_ekd_, 'spatial-pattern') if plot_pattern and hasattr(self,'A_ekd_') else (self.W_ekd_, 'spatial-filter')
        plot_factoredmodel(sp[0].reshape((-1,sp[0].shape[-1])), None, evtlabs=evtlabs, spatial_filter_type=sp[1], ncol=1, ch_names=ch_names, **kwargs)
    ...
    def testcase(self, dur=3, fs=100, blksize=10):
        ...
        X_TSd, Y_TSy, st, A, B = testSignal(d=3, tau=1, nY=1, noise2signal=.0, nTrl=100, nSamp=300, isi=0, classification=False, induced=-1)
        fs=100
...
class FilterBankCommonSpatialPatterner(CommonSpatialPatterner):
    def __init__(self, nfilt_per_class:int=3, filterbank=None, mean:bool=True, compress_feature_dim=False, spoc:bool=False, reg:float=1e-6, rcond:float=1e-6, verb=1):
        super().__init__(nfilt_per_class,mean,compress_feature_dim,spoc,reg,rcond,verb)
        self.filterbank=filterbank
    
    def get_filterbank_class_covariances(self,X_TSd,Y_TSye,filterbank=None,spoc:bool=False, y0_is_true:bool=True):
        pass
...
class Resampler(BaseEstimator, ModifierMixin):
    ...
    """    
    def __init__(self, axis:int=1, fs:float =250, fs_out:float =60, mode:str='sample', resample_y:str='max', nsamp:int=0, verb:int=0):
    ...
    def fit(self, X, y=None, fs:float =None):
        ...
        if self.verb>1 : print("resample: {}->{}hz rsrate={}".format(self.fs_, self.fs_out_, self.resamprate_))
        ...
        if self.resamprate_ > 1:
            ...
            if Y is not None:
                ...
                for i in range(len(idx)):
                    sampIdx = slice(int(idx[i]),int(idx[i+1])) if i<len(idx)-1 else slice(int(idx[i]),Y.shape[self.axis])
                    rY[:,i,...] = pool_axis(Y[:,sampIdx,...],self.resample_y,self.axis) # decimate Y (trl, samp, y)
                Y = rY
...
class ButterFilterAndResampler(BaseEstimator, ModifierMixin):
    ...
    def __init__(self, filterband=((0,5),(5,-1)), order:int=6, axis:int=1, fs:float =None, fs_out:float =None, ftype:str='butter', filter_y:bool=False, resample_y:str='max', verb:int=0):
        """[summary]
    ...
    def fit(self, X, y=None, fs:float =None, zi=None):
        ...
        _, self.sos_, self.zi_ = butter_sosfilt(X[0,...], self.filterband, self.fs_, order=self.order, axis=self.axis-1, zi=zi, ftype=self.ftype)

        if self.filter_y:
            if y.ndim<3:
                y = y.reshape( (1,)*(3-y.ndim) + y.shape)
            _, self.sos_y_, self.zi_y_ =  butter_sosfilt(y[0,...], self.filterband, self.fs_, order=self.order, axis=self.axis-1, ftype=self.ftype)
        else:
...
class PreprocessPipeline(BaseEstimator):
    ...
    def fit_modify(self,X,Y,until_stage:int=None, verb:int=0,**kwargs):
        for i in range(len(self.stages)):
            ...
            if verb>0: print("{:d}) {}  X={} Y={}".format(i,name,X.shape,Y.shape))
        return (X,Y)
    ...
    def modify(self,X,Y, until_stage:int=None, verb:int=0):
        for i,(name,stage) in enumerate(self.stages):
            ...
            if self.verb>1 : print("{}) {}  X={} Y={} -> ".format(i,name,X.shape,Y.shape))
    ...
    def score(self,X,Y):
        ...
        for pi,(name,stage) in enumerate(self.stages[:-1]):
            if self.verb>0 : print("{}) {}  X={} Y={} -> ".format(pi,name,X.shape,Y.shape))
            if hasattr(stage, 'predict'): # generate final predicted Y
                X = stage.predict(X, Y)
            elif hasattr(stage, 'modify'): # change X and Y
                X,Y = stage.modify(X, Y)
                ...
                X = stage.transform(X, y=Y)
            if self.verb>0 : print("        X={} Y={} ".format(X.shape,Y.shape))
    ...
    for pi,stage in enumerate(pipeline):
        ...
        if isinstance(args,dict):
            ...
            if transformer is None:
                try:
                    transformer = import_and_make_class("mindaffectBCI.decoder.preprocess_transforms." + stagename,**args)
                except:
                    pass
