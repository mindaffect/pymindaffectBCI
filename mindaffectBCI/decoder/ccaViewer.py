...
def run(ui: UtopiaDataInterface=None, maxruntime_ms: float=np.inf, timeout_ms:float = 500, tau_ms: float=500,
    ...
    while ui.getTimeStamp() < t0+maxruntime_ms:
        ...
        Y_Sye = stim2event(M_Sy[:,usedY], evtypes=evtlabs, axis=-2, 
        ...
        J, W_mkd, R_mket = multipleCCA(Cxx_dd, Cxy_yetd, Cyy_yetet, reg=reg, rank=rank)
...
if __name__=='__main__':
    ...
    if args.savefile is not None:
        ...
        ppfn = butterfilt_and_downsample(order=6, stopband=args.stopband, fs_out=args.out_fs)
