...
from sklearn.model_selection import ShuffleSplit, cross_val_score, GroupShuffleSplit, StratifiedKFold, GroupKFold, LeavePGroupsOut
...
def run_pipeline(pipename,pipeline,X,Y,group,cv=30,fs=160,ch_names=None, random_state=42, dsname=None, train_size=30):
    # generate a deterministic data split
    ngroup = len(np.unique(group))
    cv = GroupShuffleSplit(cv, train_size=train_size, random_state=random_state)
    #cv = LeavePGroupsOut(n_groups=max(ngroup//30,1))
...
def run_analysis(pipelines, label:str='', cv=30, tmin=1, tmax=2, lfreq=7, hfreq=30, subjects=None,
                 event_id=dict(hands=2,feet=3),runs=[6,10,14]):
    if subjects is None: subjects = np.arange(1,110)
    scores=dict()
    for pipename,(pipeline,clsfr) in pipelines:
        datasets = dict()
        for subject in subjects:
            ...
            # run the subject specific pipeline
            Xpp = pipeline.fit_transform(X,Y)
            # accumulate into the multi-subject training set
            datasets[subject]=(Xpp,Y,fs,ch_names)
        
        # got the pre-processed per-subject data
        # combine into a single dataset and compute the group label
        N = len(datasets)
        print("Loaded {} datasets.  Merging...".format(N))
        Xs,Ys,groups = [],[],[]
        for s,(X,Y,fs,ch_names) in datasets.items():
            Xs.append(X)
            Ys.append(Y)
            groups.append([s]*X.shape[0])
        Xs = np.concatenate(Xs,0)
        Ys = np.concatenate(Ys,0)
        groups = np.concatenate(groups,0)

        # remove the raw to free up the ram
        del datasets, X, Y, Xpp, epochs
        gc.collect()

        print("Merged {} datasets.  Training...".format(N))
        # run the cross-validated performance estimation
        for trn_sz in train_size:
            pp = pipename + "{}trn".format(trn_sz)
            pscores=run_pipeline(pp,clsfr,Xs,Ys,groups,cv=cv,ch_names=ch_names,train_size=trn_sz)
            if not pp in scores: scores[pp]=[]
            scores[pp].append(pscores.tolist())
        ...
        for k,v in scores.items():
            ...
            print("{:30s} : {:5.3f} (N={:d})".format(k,np.mean(ave),N))
            score_ave[k]=(ave,N,v)
...
stdamp = [Pipeline([
                ...
                ('std',UnitAmp(axis=2)), # TSd 
                ('amp',AverageAmplitude(axis=1,center=True,log=True)), # Td
                ('vec', Vectorizer()),  # Tf
                ]),
...
whtfb2hzALRCV =  [Pipeline([
                ...
                ('amp',AverageAmplitude(axis=2,center=False,log=True))
                ]),
                AlternatingLRCV(axes=[(1,),(2,)])
                ]
...
if __name__=='__main__':
    ...
    label='xsubj_{}_{}trn'.format("_".join([p[0] for p in pipelines]), train_size)
    run_analysis(pipelines,label=label,cv=30,subjects=list(range(1,60)))